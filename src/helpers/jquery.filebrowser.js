...
(function ($, undefined) {
  ...
  $.prototype.browse = function (options) {
    ...
    function mousemove(e) {
      if (selection) {
        ...
        var $selected = $li.filter(function () {
          ...
          return (
            rect.top + rect.height > selection_rect.top &&
    ...
    function click(e) {
      ...
      } else if (!$target.closest(".browser-menu").length || $target.closest(".browser-menu li").length) {
        hide_menus();
      }
    }
    ...
    function process_textarea(remove) {
      ...
      if ($li.hasClass("rename")) {
        ...
        if (new_name != old_name) {
          self._rename(self.join(path, old_name), self.join(path, new_name)).then(refresh_same);
        }
    ...
    function make_menu(context, submenu) {
      ...
      Object.keys(context).forEach(function (name) {
        var $li = $('<li class="' + class_name(name) + '"><div>' + name + "</div></li>").appendTo($ul);
    ...
    if (this.data("browse")) {
      ...
      $content
        .on("dblclick.browse", "li", function (e) {
          var $li = $(this);
      ...
      self.on("click.browse", function (e) {
        ...
        if (!$target.is("textarea")) {
          $content.find("li.rename,li.new").find("textarea").each(process_textarea);
        }
      });
      self
        .on("dragover.browse", ".content", function (event) {
          if (event.originalEvent) {
            event = event.originalEvent;
          }
        ...
        .on("drop.browse", function (e) {
          ...
          if (is_file_drop(e)) {
            ...
            if (drag.selection) {
              promise = $.when.apply(
                $,
                selected[settings.name].map(function (src) {
      ...
      $.extend(self, {
        ...
        create: function (type, path) {
          ...
          if (path == undefined) {
            var $li = $(
              ['<li class="new ' + _class + '" draggable="true">', "  <span></span>", "  <textarea/>", "</li>"].join("")
            ).appendTo($ul);
            scroll_to_bottom();
            $li
              .find("textarea")
        ...
        paste: function (cut) {
          function process(widget, fn) {
            return $.when.apply(
              $,
              copy.contents.map(function (src) {
        ...
        join: function () {
          ...
          var path = paths
            ...
            .filter(Boolean)
            .join(settings.separator); // + settings.separator;
